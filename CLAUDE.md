...
3. **クロスレビュー** — セキュリティや設計に触れる変更は、Orcaのターミナル経由でCodexを起動し第三者レビューさせる(レポートはリポジトリ直下にmdで書かせる)。指摘は鵜呑みにせず妥当性を検証し、**対応/受容を明示的に仕分ける**(受容理由はADRかコミットメッセージに残す)
...
- シークレットの値をチャット・コミット・ログに出さない。値の受け渡しは `pbcopy` / stdinパイプで行い、露出したら即 `wrangler secret put AUTH_TOKEN` でローテーション
